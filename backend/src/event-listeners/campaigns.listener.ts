...
export class CampaignsListener {
  ...
  async handleFlowAssigned(event: CampaignFlowAssignedEvent) {
    console.log(
      "Handling CampaignFlowAssignedEvent::",
      JSON.stringify(event, null, 2),
    );
    // const campaign = await this.campaignsService.findOne(event.campaignId);
    ...
    console.log("Fetched campaign:", JSON.stringify(campaign, null, 2));
    ...
    console.log("Flow step IDs to assign:", flowStepIds);
    Promise.all(
      flowStepIds?.map((id) => {
